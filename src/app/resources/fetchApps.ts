...
async function getApp(appName: string) {
         ...
         if (cache[appName]) {
                  ...
                  const {data: mainAppData}: any = await fetch(`https://github.com/ahqsoftwares/ahq-store-data/raw/main/database/${appName}.json`, {
                           method: "GET",
                           responseType: 1
                  });

                  const {data: authorData}: any = await fetch(`https://github.com/ahqsoftwares/ahq-store-data/raw/main/database/user${mainAppData.author.id}.json`, {
                           method: "GET",
                           responseType: 1
                  });
